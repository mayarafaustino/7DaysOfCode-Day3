...
function abrirStack(stack){
    if (stack == "front-end"){
        const msg = `<p>Parabéns front-ender! Vamos seguir sua formação.</p>
        <p>Deseja aprender React ou Vue?</p>
        <input type="text" id="react-vue">
        <button type="button" onclick="primeiraTec('react-vue')">Seguir</button>`
        divOpcao.innerHTML = msg;
    }
    if (stack == "back-end"){
        const msg = `<p>Parabéns back-ender! Vamos seguir sua formação.</p>
        <p>Deseja aprender C# ou Java?</p>
        <input type="text" id="c-java">
        <button type="button" onclick="primeiraTec('c-java')">Seguir</button>`
        divOpcao.innerHTML = msg;
    }
}
...
function escolherEspecializacao() {
    const msg = `<p>Muito bem Dev! Agora precisa decidir se continuará como um desenvolvedor ${stackEscolhida} ou se tornará fullstack!</p>
    ...
    <button type="button" onclick="selecionarEspecializacao()">Seguir</button>`
...
function escolherOutrasTecs() {
    ...
    <button type="button" onclick="abrirInputTec()">Seguir</button>`
    ...
    for (let i = 0; i < radio3.length; i++){
        if (radio3[i].checked == true){
            ...
            if (alternativa == "sim") {
                ...
                <button type="button" onclick="guardarTec()">Seguir</button>`
